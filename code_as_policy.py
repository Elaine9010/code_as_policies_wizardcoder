## The Script defines the main objects and functions for code-as-policy
...
def setup_LMP(env, llm, cfg_tabletop):
    # LMP env wrapper
    ...
    lmp_fgen = LMPFGen(llm, cfg_tabletop["lmps"]["fgen"], fixed_vars, variable_vars)
...
def auto_correct_python_code(code_str):
    # auto correct python code formatting
    with open("tmp.py", "w") as f:
        f.write(code_str)

    autogen8_cmd = "autogen8 --in-place --aggressive tmp.py"
    try:
        os.system(autogen8_cmd)
    except:
        print("ERROR: autogen8 failed to run.")
        return code_str
  
    # read the auto-corrected code
...
def exec_safe(code_str, gvars=None, lvars=None):
    ...
    # auto correct python code formatting
    code_str = auto_correct_python_code(code_str)

    #print(f"\n[DEBUG] exec_safe: available global variables: {custom_gvars.keys()}, and local variables: {lvars.keys()}")
            ...
            if ("block" in obj_name) or ("bowl" in obj_name):
                ...
                if object_type == "block":
                    ...
                    object_id = pybullet.loadURDF(
                        "bowl/bowl.urdf", object_position, useFixedBase=1
                    )
...
class LMP:
    ...
    def __call__(self, query, context="", **kwargs):
        ...
        # wizardCoder backbone
        stop_tokens = self._stop_tokens
        sampling_params = SamplingParams(
            temperature=1, top_p=1, max_tokens=self._cfg["max_tokens"], stop=stop_tokens
        )
        completions = self._lmp_fgen.llm.generate(prompt, sampling_params)
        code_str = completions[0].outputs[0].text.strip()
        print(f"DEBUG: LLM Generation Done.")
...
class LMPFGen:
    def __init__(self, llm, cfg, fixed_vars, variable_vars):
        self._cfg = cfg
    ...
    def create_f_from_sig(
        ...
        #print(f"DEBUG: LLM Generation Started.\ncreate_f_from_sig generated prompt:\n{prompt}\n")
        # openai gpt backbone
        #while True:
        #    try:
        ...
        completions = self.llm.generate(prompt, sampling_params)
        f_src = completions[0].outputs[0].text.strip()
        print(f"created f_src:\n{f_src}\n")
       
        if other_vars is None:
            other_vars = {}
        gvars = merge_dicts([self._fixed_vars, self._variable_vars, other_vars])
        lvars = {}
        #print(f"DEBUG: Executing exec_safe(f_src, gvars, lvars)")
    ...
    def create_new_fs_from_code(
        ...
        # auto collect code string
        code_str = auto_correct_python_code(code_str)

        f_parser.visit(ast.parse(code_str))
        #print("f_assigns: ", f_assigns)
...
class LMP_wrapper:
    ...
    def get_obj_positions_np(self, obj_names):
        return [self.env.get_obj_pos(obj_name)[:2] for obj_name in obj_names]
...
#         file_name =  name + f'_{prompt_version}'
...
#         file1.close()
#     #print("Current prompt version is: ", prompt_version)
